    ...
    useEffect(() => {
        if(assignedFor === user._id.toString()){
            switch (status){
                case 'new':
                    setSecondButton(
                    <button className={styles.button3} onClick={() => { changeStatus('work', id, '/tasks', currentContent, filters)}}>
                        Zacznij Prace <FontAwesomeIcon style={{marginLeft: '1rem'}} icon={faCheck}/>
                    </button>)
                    break;
                case 'work':
                    setSecondButton(
                    <button className={styles.button4} onClick={() => { changeStatus('end', id, '/tasks', currentContent, filters)}}>
